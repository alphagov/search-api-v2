module Metrics
  class Evaluation
    ...
    def record_evaluations(evaluation_result, month, month_name, year, dataset)
      metrics.each { |key, registry| record_evaluation(key, registry, month, month_name, year, dataset, evaluation_result) }
    end
  ...
  private
    ...
    def initialize
      @doc_recall = registry.gauge(
        ...
        labels: %i[top month month_name year dataset],
      )
    ...
    def record_evaluation(key, registry, month, month_name, year, dataset, evaluation_result)
      TOP_K_LEVELS.each do |k|
        value = evaluation_result.dig(key, :"top_#{k}")
        registry.set(value, labels: { top: k, month:, month_name:, year:, dataset: }) if value.present?
      end
    end
  end
end
