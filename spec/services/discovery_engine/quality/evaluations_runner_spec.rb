RSpec.shared_context "with sample query sets and evaluations set up" do |table_id|
...
RSpec.shared_examples "creates sample query sets and evaluations" do |method, table_id|
  before do
    evaluations_runner.send(method)
  end

  it "fetches sample query sets for this month and the month before last" do
...
RSpec.describe DiscoveryEngine::Quality::EvaluationsRunner do
  ...
  context "when a table id is passed in" do
    ...
    include_context "with sample query sets and evaluations set up", "explicit"

    describe "#upload_detailed_metrics" do
      it_behaves_like "creates sample query sets and evaluations", :upload_detailed_metrics, "explicit"
    ...
    describe "#upload_and_report_metrics" do
      it_behaves_like "creates sample query sets and evaluations", :upload_and_report_metrics, "explicit"
  ...
  context "when no table_is provided" do
    subject(:evaluations_runner) { described_class.new }

    let(:sample_query_set) { double("sample_query_set", name: "name") }
    let(:sample_query_sets_last_month) { double("sample_query_sets_last_month") }
    let(:sample_query_sets_month_before_last) { double("sample_query_sets_month_before_last") }
    let(:last_month_partition_date) { Date.new(1979, 10, 1) }
    let(:evaluation) do
      double("evaluation",
             list_evaluation_results: "detailed_metrics",
    ...
    before do
      allow(DiscoveryEngine::Quality::SampleQuerySets)
        .to receive(:new)
        .with(:last_month)
        .and_return(sample_query_sets_last_month)

      allow(sample_query_sets_last_month)
        .to receive(:all)
        .and_return([sample_query_set, sample_query_set, sample_query_set, sample_query_set])

      allow(DiscoveryEngine::Quality::SampleQuerySets)
        .to receive(:new)
        .with(:month_before_last)
        .and_return(sample_query_sets_month_before_last)

      allow(sample_query_sets_month_before_last)
        .to receive(:all)
        .and_return([sample_query_set, sample_query_set, sample_query_set, sample_query_set])
    ...
    describe "#upload_detailed_metrics" do
      it "fetches sample query sets for all tables, for this month and the month before last" do
        evaluations_runner.upload_detailed_metrics

        expect(sample_query_sets_month_before_last)
          .to have_received(:all)

        expect(sample_query_sets_last_month)
          .to have_received(:all)
      end

      it "creates an evaluation of all sample query sets" do
        evaluations_runner.upload_detailed_metrics

        expect(DiscoveryEngine::Quality::Evaluation)
          .to have_received(:new)
          .with(sample_query_set).exactly(8).times
      end
    end
  end
  describe "#upload_and_report_metrics" do
