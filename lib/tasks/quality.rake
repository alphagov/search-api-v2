...
namespace :quality do
  ...
  task setup_sample_query_sets: :environment do
    ...
    sets = DiscoveryEngine::Quality::SampleQuerySets.new(month_interval).sets
    sets.each(&:create_and_import)
  end
  ...
  task report_quality_metrics: :environment do
    ...
    sample_query_sets = month_intervals.transform_values do |month_interval|
      DiscoveryEngine::Quality::SampleQuerySets.new(month_interval).sets
    end
    ...
    sample_query_sets.each do |month_label, sets|
      sets.each do |set|
        set.create_and_import

        e = DiscoveryEngine::Quality::Evaluation.new(set.id).fetch_quality_metrics
